...
const DEFAULTS = {
  ...
  phone: '+91-9324796576',
  phone2: '',
  email: 'thedreamresidency.mumbai@gmail.com',
...
async function initDefaults() {
  ...
  const blankFills = [
    ...
    ['smtp_port', '587'],
    ['smtp_user', 'ac4c53001@smtp-brevo.com'],
    ['smtp_pass', ''],
...
async function sendMail({ to, subject, html, text }) {
  try {
    ...
    const host = await getSetting('smtp_host', DEFAULTS.smtp_host);
    const user = await getSetting('smtp_user', DEFAULTS.smtp_user);
    const pass = await getSetting('smtp_pass', DEFAULTS.smtp_pass);
    const port = parseInt(await getSetting('smtp_port', DEFAULTS.smtp_port));
...
async function startServer() {
  ...
  // ── GOOGLE SHEETS WEBHOOK (Sheet → Database) ──────────────────────────────
  // Receives edits from any monthly tab in the spreadsheet.
  // Row columns: Booking ID | Submitted Date | Full Name | Email | Phone |
  //   Guests | Rooms Required | Room Type | Room Number |
  //   Check-In Date | Check-Out Date | Nights | Total Amount (₹) | Status | Notes
  app.post('/api/sheets-webhook', express.json(), async (req, res) => {
    try {
      const secret = req.headers['x-webhook-secret'];
      const expectedSecret = process.env.SHEETS_WEBHOOK_SECRET || 'dream-sheets-secret';
      if (secret !== expectedSecret) return res.status(401).json({ error: 'Unauthorized' });

      const { action, tab, row } = req.body || {};
      if (!row) return res.status(400).json({ error: 'Missing row data' });

      const bookingId = String(row['Booking ID']     || '').trim();
      const name      = String(row['Full Name']       || '').trim();
      const email     = String(row['Email']           || '').trim();
      const phone     = String(row['Phone']           || '').trim();
      const guests    = parseInt(row['Guests'])        || 1;
      const roomType  = String(row['Room Type']       || '').trim();
      const roomNum   = String(row['Room Number']     || '').trim();
      const checkIn   = String(row['Check-In Date']   || '').trim();
      const checkOut  = String(row['Check-Out Date']  || '').trim();
      const status    = String(row['Status']          || 'pending').trim().toLowerCase();
      const notes     = String(row['Notes']           || '').trim();

      if (!name || !phone || !checkIn || !checkOut) {
        return res.status(400).json({ error: 'Full Name, Phone, Check-In Date and Check-Out Date are required' });
      }

      // If Booking ID is a confirmed booking ID (numeric), update bookings table
      if (bookingId && !bookingId.startsWith('REQ-') && /^\d+$/.test(bookingId)) {
        if (action === 'update_status') {
          await db.run('UPDATE bookings SET status=? WHERE id=?', [status, bookingId]);
        } else {
          await db.run(
            `UPDATE bookings SET guests=?, room_type=?, room_number=?, check_in=?, check_out=?, status=?, notes=? WHERE id=?`,
            [guests, roomType, roomNum, checkIn, checkOut, status, notes, bookingId]
          );
        }
        return res.json({ ok: true, action: 'booking_updated', id: bookingId });
      }

      // Otherwise treat as a booking request (REQ-N or no ID)
      const reqId = bookingId.startsWith('REQ-') ? bookingId.replace('REQ-', '') : null;

      if (action === 'update_status') {
        if (reqId) {
          await db.run('UPDATE booking_requests SET status=? WHERE id=?', [status, reqId]);
        } else {
          await db.run('UPDATE booking_requests SET status=? WHERE phone=? AND check_in=?', [status, phone, checkIn]);
        }
        return res.json({ ok: true, action: 'status_updated' });
      }

      // Full upsert
      const existing = reqId
        ? await db.get('SELECT id FROM booking_requests WHERE id=?', [reqId])
        : await db.get('SELECT id FROM booking_requests WHERE phone=? AND check_in=?', [phone, checkIn]);

      if (existing) {
        await db.run(
          `UPDATE booking_requests SET name=?, email=?, check_out=?, guests=?, room_type=?, status=?, message=? WHERE id=?`,
          [name, email, checkOut, guests, roomType || roomNum, status, notes, existing.id]
        );
        return res.json({ ok: true, action: 'booking_request_updated', id: existing.id });
      } else {
        const r = await db.run(
          `INSERT INTO booking_requests (name, phone, country_code, email, check_in, check_out, guests, room_type, status, rooms_required, message)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [name, phone, '', email, checkIn, checkOut, guests, roomType || roomNum, status, 1, notes]
        );
        return res.json({ ok: true, action: 'booking_request_created', id: r.lastInsertRowid });
      }
  ...
  // ── FOLDER IMAGES (public, whitelisted) ────────────────────────────────────
  app.get('/api/folder-images', (req, res) => {
    const ALLOWED = {
      flyer:    'public/img/flyer',
      about:    'public/img/about',
      metro:    'public/img/connectivity/metro',
      railway:  'public/img/connectivity/railway',
      stations: 'public/img/connectivity/stations',
      suite:    'public/img/rooms/suite',
      deluxe:   'public/img/rooms/deluxe',
      twin:     'public/img/rooms/twin',
    };
    const folder = String(req.query.folder || '').toLowerCase().replace(/[^a-z]/g, '');
    const rel = ALLOWED[folder];
    if (!rel) return res.status(400).json({ images: [] });
    const dir = path.join(__dirname, rel);
    const EXT = /\.(jpg|jpeg|png|gif|webp|avif|svg)$/i;
    try {
      if (!fs.existsSync(dir)) return res.json({ images: [] });
      const files = fs.readdirSync(dir).filter(f => EXT.test(f)).sort();
      res.json({ images: files.map(f => '/' + rel.replace('public/', '') + '/' + f) });
  ...
  app.post('/api/request-booking', async (req, res) => {
    try {
      ...
      // Sync to Google Sheets — monthly tab routing (fire-and-forget)
      sheetsSync.syncBookingRequest({
        id:           r.lastInsertRowid,
        name,
        email:        email || '',
        phone:        fullPhone,
        guests:       parseInt(guests) || 1,
        rooms_required: parseInt(rooms_required) || 1,
        room_type:    room_type || '',
        check_in,
        check_out,
        message:      message || '',
  ...
  app.post('/api/booking-requests/:id/confirm', requireAdmin, async (req, res) => {
    try {
      ...
      // Sync confirmed booking to the correct monthly tab (fire-and-forget)
      sheetsSync.syncConfirmedBooking({
        booking_id:    bk.lastInsertRowid,
  ...
  app.post('/api/manual-booking', requireAdmin, async (req, res) => {
    try {
      ...
      // Sync walk-in booking to the correct monthly tab (fire-and-forget)
      sheetsSync.syncConfirmedBooking({
        booking_id:    bk.lastInsertRowid,
  ...
  // ── GOOGLE PLACE RATING (live, cached 6h) ─────────────────────────────────
  let _ratingCache = null, _ratingCacheAt = 0;
  app.get('/api/google-rating', async (req, res) => {
    try {
      const apiKey = process.env.GOOGLE_MAPS_API_KEY;
      if (!apiKey) return res.json({ rating: null, total: null, source: 'no_key' });

      const now = Date.now();
      if (_ratingCache && now - _ratingCacheAt < 6 * 3600 * 1000) {
        return res.json({ ..._ratingCache, source: 'cache' });
      }

      // Use Find Place From Text to get live rating
      const query = encodeURIComponent('The Dream Residency Andheri East Mumbai');
      const fields = 'rating,user_ratings_total,place_id';
      ...
      if (!candidate) return res.json({ rating: null, total: null, source: 'not_found' });

      _ratingCache = { rating: candidate.rating || null, total: candidate.user_ratings_total || null };
      _ratingCacheAt = now;
      res.json({ ..._ratingCache, source: 'live' });
    } catch (e) {
      res.json({ rating: null, total: null, source: 'error', error: e.message });
    }
  });
